...
test("service worker preloads the booth shell for repeat slow-connection launches", () => {
  assert.ok(serviceWorker.includes('const APP_SHELL_CACHE = "pb-app-shell-v5"'));
...
test("service worker serves manually cached Cloudinary artwork", () => {
  ...
  assert.ok(serviceWorker.includes('event.request.destination === "audio"'));
  assert.ok(serviceWorker.includes("mp3|wav|m4a|aac|ogg|oga"));
});
