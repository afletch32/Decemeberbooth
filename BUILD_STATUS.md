...
- Added an admin-only Guest Filter Editor in Capture settings. It exposes safe beauty and lighting controls per guest preset, saves overrides in the shared theme payload, and synchronizes them through the existing theme endpoint.
- Imported overlay and template artwork can now be adjusted directly from the Asset Library: drag and resize its photo window, choose fill or whole-photo fit, and save normalized window data to the shared library and linked theme assets. The legacy overlay builder remains untouched.
